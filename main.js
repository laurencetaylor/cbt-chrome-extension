...
const renderRandomLeafSvg = () => {
    ...
    svgPlaceholder.setAttribute('data', `./assets/plant-${randomNumber}${darkModeSuffix}.svg`);
};
...
const toggleDarkMode = (darkModeToggle, darkModeText, lightModeText) => {
    ...
    darkModeToggle.innerHTML = isDarkMode ? lightModeText : darkModeText;

    const leafSvg = document.querySelector('.leaf-svg');

    const data = leafSvg.getAttribute('data');
    const newData = data.split(':')[0] + (isDarkMode ? ':' : ':darkmode') + '.svg';
